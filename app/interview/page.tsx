...
export default function InterviewVoiceDemo() {
  ...
  // Add: helper to (re)bind stream to the <video> when it (or the UI) becomes available
  const attachStreamToVideo = async () => {
    ...
    const onLoaded = async () => {
      try {
        ...
        /* ignore autoplay block; user action will play */
      }
    };
  ...
  // Bind video element when stream or UI (status) changes
  useEffect(() => {
    attachStreamToVideo();
    return () => {
      // no-op; stopping handled elsewhere
    };
  }, [videoStream, status]);
