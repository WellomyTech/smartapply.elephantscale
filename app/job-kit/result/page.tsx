...
export default function JobKitResultPage() {
  ...
  useEffect(() => {
    const jobUrl =
      localStorage.getItem("job_url") ||
      localStorage.getItem("jobLink") ||
      localStorage.getItem("job_link") ||
      null;
    ...
    console.log(
      "Resume text:",
      resume ? `${resume.substring(0, 100)}...` : "null"
    );
    console.log(
      "Cover letter text:",
      coverLetter ? `${coverLetter.substring(0, 100)}...` : "null"
    );
  ...
  const downloadResumeDocx = async () => {
    ...
    try {
      const response = await fetch(
        `${API_KEY}download-custom-resume-docx?report_id=${reportId}`
      );
      if (!response.ok) {
  ...
  const downloadCoverLetterDocx = async () => {
    ...
    if (!coverLetter) {
      showStatus(
        "No cover letter found. Please regenerate your documents.",
        "error"
      );
      return;
    }
            ...
            <Card className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border-green-200/30 backdrop-blur-sm rounded-2xl shadow-xl max-w-3xl mx-auto">
              <CardContent className="p-8">
                ...
                <p className="text-lg text-gray-600 dark:text-gray-300 mb-6 max-w-2xl mx-auto">
                  ...
                  download. These documents have been tailored specifically for
                  your target role to maximize your chances of success.
                </p>
                  ...
                  {jobLink ? (
                    ...
                    <div className="text-center">
                      <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
                        No specific job link found. You can now apply to any job
                        posting with your optimized documents.
                      </p>
                      <Button
