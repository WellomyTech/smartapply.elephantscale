            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setTimeout(() => { router.push('/job-kit/result') }, 1000)
                } catch (error) {
                  console.error(error)
...
//   // Update workedOn after getting a result  ⟵ REPLACE THIS EFFECT
// useEffect(() => {
//   if (!result) return
...
//   // Build a quick lookup from skills_match
// const skillFlags: Record<string, { in_job: boolean; in_resume: boolean }> = {}
// result?.skills_match?.forEach(s => {
//   if (!s?.skill) return
//   skillFlags[s.skill] = { in_job: !!s.in_job, in_resume: !!s.in_resume }
...
//   const compareDisabled = loading || !hasResume || !description.trim() || !jobUrl.trim() || !!jobUrlError || !jobTitle.trim() || !jobCompany.trim()
...
//                       </TableBody> */}
//                       <TableBody>
//   {result.gaps.map((gapSkill, idx) => {
//     const flags = skillFlags[gapSkill] ?? { in_job: true, in_resume: false }

//     return (
//       <TableRow key={`${gapSkill}-${idx}`}>
//         <TableCell className="font-medium">{gapSkill}</TableCell>
...
//           >
//             <div className="flex items-center space-x-2">
//               <RadioGroupItem id={`yes-${idx}`} value="true" />
//               <Label htmlFor={`yes-${idx}`}>Yes</Label>
//             </div>
//             <div className="flex items-center space-x-2">
//               <RadioGroupItem id={`no-${idx}`} value="false" />
