...
export default function JobInfoPage() {
  ...
  const report_id = Array.isArray(params.report_id)
    ? params.report_id[0]
    : params.report_id;
      ...
      .then((data) => {
        if (data?.report) {
          ...
          localStorage.setItem(
            "report_id",
            report.id || report.report_id || (report_id as string)
          );
        }
        setLoading(false);
      })
  ...
  async function handleGenerateResume() {
    ...
    const jobInfoToSend = jobData.job_link
      ? jobData.job_link
  ...
  const downloadResumeDocx = async () => {
    ...
    try {
      const response = await fetch(
        `${API_KEY}download-resume-docx?report_id=${reportId}`
      );
  ...
  const downloadCoverLetterDocx = async () => {
    ...
    if (!coverLetter) {
      showStatus(
        "No cover letter found. Please generate one first.",
        "warning"
      );
      return;
    }
  ...
  const gaps = safeJsonArray(jobData.gaps);
  const skills_match = Array.isArray(jobData.skills_match)
          ...
          <div className="flex flex-col items-center gap-6 mt-10">
            <div className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-md rounded-2xl border border-white/20 dark:border-gray-700/30 p-8 shadow-xl text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6">
                Resume and cover letter not found for this job. Let's analyze
                the skills match and generate personalized documents.
              </p>
              <Button
                size="lg"
