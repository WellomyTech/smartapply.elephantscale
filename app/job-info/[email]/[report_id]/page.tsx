...
export default function JobInfoPage() {
  ...
  const reportParam = Array.isArray(params.report_id)
    ? params.report_id[0]
    : (params.report_id as string | undefined);
  ...
  const resumeDisplay = useMemo(
    () => normalizeNewlines(resumeText ?? ""),
    [resumeText]
  );
  const coverDisplay = useMemo(
    () => normalizeNewlines(coverLetterText ?? ""),
    [coverLetterText]
  );
  ...
  useEffect(() => {
    ...
    fetch(
      `${API_URL}user-dashboard?user_email=${emailParam}&report_id=${reportParam}`
    )
      .then((res) => res.json())
      .then((data) => {
        if (data?.report) {
          ...
          try {
            if (typeof report.selected_skills === "string")
              report.selected_skills = JSON.parse(report.selected_skills);
          } catch {}
          setJobData(report);
          if (report?.id || report?.report_id || reportParam) {
            localStorage.setItem(
              "report_id",
              String(report.id || report.report_id || reportParam)
            );
          }
        }
        setLoading(false);
  ...
  useEffect(() => {
    if (!jobData) return;
    if (jobData.updated_resume)
      setResumeText(normalizeNewlines(String(jobData.updated_resume)));
    if (jobData.cover_letter)
      setCoverLetterText(normalizeNewlines(String(jobData.cover_letter)));
  ...
  useEffect(() => {
    ...
    const selected = new Set(
      safeJsonArray(jobData.selected_skills).map((s) =>
  ...
  const skills_match: SkillsMatchItem[] = Array.isArray(jobData.skills_match)
  ...
  async function handleGenerateResume() {
    ...
    if (selectedSkills.length === 0) {
      showStatus(
        "Please select at least one skill you have worked on.",
        "warning"
      );
      return;
    }
    ...
    setGenerating(true);
    showStatus(
      "Generating your personalized resume and cover letter...",
      "loading"
    );
      ...
      if (!updated && email && rid) {
        try {
          const refRes = await fetch(
            `${API_URL}user-dashboard?user_email=${encodeURIComponent(
              email
  ...
  async function saveEdits(which: "resume" | "cover") {
    ...
    if (which === "resume") {
      ...
      const cover = normalizeNewlines(
        ((editingCover ? coverDraft : coverLetterText || "") || "").trim()
      );

      setSavingResume(true);
      showStatus("Saving resume to server...", "loading");
      const ok = await updateJobStatus({
        reportId: rid,
        resume_text: next,
        cover_letter: cover,
      });
    ...
    const resume = normalizeNewlines(
      ((editingResume ? resumeDraft : resumeText || "") || "").trim()
    );
  ...
  const downloadResumeDocx = async () => {
    ...
    try {
      ...
      if (text) {
        ...
        const tryPost = await fetch(`${API_URL}generate-resume-docx`, {
          method: "POST",
          body: fd,
        });
        if (tryPost.ok) {
  ...
  const downloadCoverLetterDocx = async () => {
    const coverLetter = (
      coverLetterText ||
      localStorage.getItem("generated_cover_letter") ||
      ""
    ).trim();
    if (!coverLetter) {
      showStatus(
        "No cover letter found. Please generate or edit first.",
        "warning"
      );
      return;
    }
  ...
  async function updateJobStatus(args: UpdateJobStatusArgs): Promise<boolean> {
    try {
      ...
      if (typeof args.applied === "boolean")
        params.append("applied", String(args.applied));
      if (typeof args.r_interview === "boolean")
        params.append("r_interview", String(args.r_interview));
      if (typeof args.resume_text === "string")
        params.append("resume_text", args.resume_text);
      if (typeof args.cover_letter === "string")
        params.append("cover_letter", args.cover_letter);
  ...
  async function handleAppliedResponse(applied: boolean) {
    ...
    showStatus(
      ok
        ? "Got it. You can update this later."
        : "Could not save status, but you can try again later.",
      ok ? "warning" : "error"
    );
  }
  ...
  function handleInterviewChoice(type: "technical" | "behavioral") {
    ...
    if (type === "technical") {
      if (rid) {
        router.push(
          `/interview?report_id=${encodeURIComponent(rid)}&type=technical`
        );
      } else {
                  ...
                  <tbody>
                    {gaps.map((gapSkill, idx) => {
                      const match = skills_match.find(
                        (s) =>
                          normalizeSkillName(s).toLowerCase() ===
                          gapSkill.toLowerCase()
                      );
                      const in_resume =
                ...
                <CardContent className="flex flex-col h-[600px] p-0">
                  ...
                  <div className="p-6 pt-0 space-y-3">
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <span>
                        Optimized with AI for maximum ATS compatibility
                      </span>
                    </div>
                    <Button
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-emerald-100 text-sm">
                          Crafted to highlight your strengths
                        </p>
                      </div>
                    </div>
                    {!editingCover ? (
                ...
                <CardContent className="flex flex-col h-[600px] p-0">
                  ...
                  <div className="p-6 pt-0 space-y-3">
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Sparkles className="h-4 w-4" />
                      <span>
                        Personalized messaging that resonates with employers
                      </span>
                    </div>
                    <Button
              ...
              <Card className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border-green-200/30 backdrop-blur-sm rounded-2xl shadow-xl max-w-3xl mx-auto">
                <CardContent className="p-8">
                  ...
                  <p className="text-lg text-gray-600 dark:text-gray-300 mb-6 max-w-2xl mx-auto">
                    Your AI-optimized resume and cover letter are ready for
                    download. These documents have been tailored specifically
                    for your target role to maximize your chances of success.
                  </p>
