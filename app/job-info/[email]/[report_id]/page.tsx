...
export default function JobInfoPage() {
  ...
  const downloadCoverLetterDocx = async () => {
    // Use report_id like resume download
    const reportId =
      localStorage.getItem('report_id') ||
      String(jobData?.id || jobData?.report_id || reportParam || '')
    ...
    try {
      const url = `${API_URL}generate-cover-letter-docx?report_id=${encodeURIComponent(reportId)}`
      ...
      const a = document.createElement('a')
      a.href = objectUrl
      a.download = `${jobData?.job_title || 'cover_letter'}.docx`
      document.body.appendChild(a)
      a.click()
      a.remove()
